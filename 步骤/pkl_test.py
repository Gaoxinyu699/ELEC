...
label0 = torch.zeros(2070)
label1 = torch.ones(1420)
label2 = torch.ones(800)
label2 *= 2

label3 = torch.zeros(400)
label4 = torch.ones(400)
...
df1 = pd.read_excel('ds1/train/第一类_train.xlsx',sheet_name='Sheet1')
df2 = pd.read_excel('ds1/train/第二类_train.xlsx',sheet_name='Sheet1')
df3 = pd.read_excel('ds1/train/第三类_polyfit_train.xlsx',sheet_name='Sheet1')
#测试集
df4 = pd.read_excel('ds1/test/第一类_test.xlsx',sheet_name='Sheet1')
df5 = pd.read_excel('ds1/test/第二类_test.xlsx',sheet_name='Sheet1')
df6 = pd.read_excel('ds1/test/第三类_polyfit_test.xlsx',sheet_name='Sheet1')
...
if CUDA:
    net = Net(input_feature=2, num_hidden=32, outputs=4).cuda()
    # net = torch.load('model0.pkl')
    #输入训练
    ...
    net = Net(input_feature=2, num_hidden=32, outputs=4).cpu()
    # net = torch.load('model0.pkl')
    #输入训练
    inputs_train = x_train.cpu()
    target_train = y_train.cpu()
    #输入测试
    inputs_test= x_test.cpu()
    target_test = y_test.cpu()
...
def draw_test(output):
    ...
    accuracy = sum(pred_y == target_y)/5290
    plt.text(1.5, -4, 'Test Accuracy=%s' % (accuracy), fontdict={'size':20, 'color':'red'})
    #plt.pause(1)
...
# a = list(arg)
# print(a)
optimizer = torch.optim.SGD(arg, lr=0.01)
auc_train = train(net, criterion, optimizer, 10000)
print(auc_train)
auc_test = test(net)
print(auc_test)
...
print("final is in : %s Seconds " % (end1 - start1))
    ...
    # if auc_train > 0.9 and auc_test > 0.8:
    #     # 保存
    #     torch.save(net, 'C:/Users/Adm/Desktop/ElecP/model/model'+str(i)+'.pkl')
