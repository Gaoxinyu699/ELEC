...
df7 = pd.read_excel('347_train.xlsx',sheet_name='Sheet1')
...
for i in range(0, len(consumption7)):
    temp7 = []
    temp7.append(consumption7[i])
    temp7.append(rmse7[i])
    res7.append(temp7)

for i in range(0, len(consumption8)):
    temp8 = []
    temp8.append(consumption8[i])
    temp8.append(rmse8[i])
    res8.append(temp8)
...
data6 = torch.tensor(res7)
data7 = torch.tensor(res8)
...
#训练
x_train = torch.cat((data0, data1, data2,data6), ).type(torch.FloatTensor)
y_train = torch.cat((label0, label1, label2,label6), ).type(torch.LongTensor)
# print(len(data6))
# print(len(label6))
# print(len(x_train),len(y_train))
#测试
x_test = torch.cat((data3, data4, data5 ,data7), ).type(torch.FloatTensor)
y_test = torch.cat((label3, label4, label5,label7), ).type(torch.LongTensor)
...
if CUDA:
    ...
    net = Net(input_feature=2, num_hidden=20, outputs=4).cpu()
    #输入训练
...
def draw_train(output):
    ...
    accuracy = sum(pred_y == target_y)/3061
